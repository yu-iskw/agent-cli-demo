...
async def enforce_mesh_auth(callback_context: CallbackContext) -> None:
    """Reject requests when the user persona lacks flight-researcher access."""
...
root_agent = Agent(
    ...
    description="Searches flights between cities for the trip-planner mesh.",
    instruction=(
        "You are the flight-researcher specialist. Use search_flights for routes. "
        "If state mesh_auth_denied is set, respond with that message only. "
        "Summarize options clearly with price and departure time."
    ),
