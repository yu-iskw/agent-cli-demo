...
- **`uv run pytest`** — Tests _code correctness_: imports work, functions return expected types, API contracts hold. Does NOT test whether the agent behaves well.
- **`agents-cli eval`** — Tests _agent behavior_: response quality, tool usage, persona consistency, safety compliance. This is what validates your agent actually works.
...
1. **Target Identification:** Clearly define the exact lines or expressions to change, based _solely_ on the user's explicit instructions.
2. **Preservation Check:** Confirm that all code, configuration values (e.g., `model`, `version`, `api_key`), comments, and formatting _outside_ the identified target remain identical.
